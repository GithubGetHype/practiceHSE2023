...
def progress(user_id):
    ...
    courses = Session.query(Progress).filter_by(user_id=user_id).all()
    courses_list = [{'course_id': course.course_id} for course in courses]
    # считаем количество правильных ответов этого пользователя для каждого курса и добавляем в словарь название курса:
    # количество ответов/всего вопросов в этом курсе
    progres = {}
    for course in courses_list:
        course_id = course['course_id']
        # Получаем название курса по course_id
        course_name = Session.query(Courses).filter_by(course_id=course_id).first().course_name
        # Получаем количество вопросов в курсе по course_id
        lessons_count = Session.query(Courses).filter_by(course_id=course_id).first().course_count
        # Получаем количество правильных ответов пользователя по course_id
        answers_count = Session.query(Progress).filter_by(user_id=user_id, course_id=course_id).count()
        progres[course_name] = f'{answers_count}/{lessons_count}'

    return render_template('my_progress.html', user=user, progress=progres)
